"""Faithful benchmark: plain Qwen3.8-27B serve (no speculative decoding).

Mirrors serve-qwen3-8.py serve path exactly: binary v0.4.0, Q4_K_XL main
model, NO draft model (the original downloaded a draft but never passed
-md), -b 512 / -ub 256. Zrok + chat-demo cells removed (localhost bench).
...
def gpu_guard():
    try:
        ...
        log("GPU", f"torch.cuda.device_count()={n}")
        if n == 0:
            log("GPU", "FATAL: no CUDA device visible")
            sys.exit(1)
        return n
    except ImportError:
        log("GPU", "torch missing, falling back to nvidia-smi")
        r = run_cmd(["nvidia-smi", "-L"], check=False)
        log("GPU", (r.stdout or "").strip() or "nvidia-smi empty")
        if r.returncode != 0:
            sys.exit(1)
        return 1
...
def ensure_assets():
    ...
    if not os.path.exists(CONFIG["MODEL_PATH"]):
        log("DOWNLOAD", f"Downloading model to {CONFIG['MODEL_PATH']}...", flush=True)
        run_cmd(
            [
                "aria2c",
                "-x",
        ...
        log("DOWNLOAD", "Model present, skipping.", flush=True)
    server_bin = os.path.join(CONFIG["BIN_DIR"], "llama-server")
    if not os.path.exists(server_bin):
        log("DOWNLOAD", "Fetching CUDA prebuilt binary (v0.4.0)...", flush=True)
...
def main():
    ...
    try:
        ...
        print("=" * 60, flush=True)
        print(f"VARIANT: {VARIANT} | binary v0.4.0 | speculative OFF", flush=True)
        for r in results:
            print(
