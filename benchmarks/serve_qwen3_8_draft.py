# Working Qwen3.8-27B serve: speculative draft-dflash on 2xT4 + zrok share.
# Proven green on Kaggle 2026-09-15. ZROK_TOKEN comes from the environment
# (dashboard injects it at push) and is never baked into this file.
...
CONFIG = {
    ...
    or "/kaggle/tmp/qwen3.8-27b-dflash-q8.gguf",
...
if not os.path.exists(CONFIG["DRAFT_MODEL_PATH"]):
    ...
    run_cmd(
        [
            ...
            "/kaggle/tmp",
            "-o",
            os.path.basename(CONFIG["DRAFT_MODEL_PATH"]),
            CONFIG["DRAFT_MODEL_URL"],
        ]
    )
else:
    log("DOWNLOAD", "Draft model binary already present. Skipping download.")
...
for _ in range(90):
    if server_proc.poll() is not None:
        with open(CONFIG["LOG_SERVER"], "r") as f:
            print(f.read())
        raise RuntimeError("llama-server process crashed during startup.")
    try:
