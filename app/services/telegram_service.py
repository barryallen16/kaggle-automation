...
class TelegramService:
    ...
    async def send_message(cls, text: str, parse_mode: str = "HTML") -> Dict[str, Any]:
        ...
        if not token or not chat_id:
            logger.warning(
                "Telegram Bot Token or User ID not configured. Message not sent."
            )
            return {
                "success": False,
                "error": "Telegram Bot Token or User ID not configured",
            }
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                if response.status_code == 200 and data.get("ok"):
                    ...
                    if (
                        "initiate conversation" in description
                        ...
                        description += " | The User ID looks invalid. Get your numeric ID from @userinfobot."
    ...
    async def send_test_message(
        ...
        chat_id = (
            test_chat_id or get_setting("telegram_chat_id", TELEGRAM_CHAT_ID) or ""
        )

        if not token or not chat_id:
            return {
                "success": False,
                "error": "Both Bot Token and User ID are required.",
            }
    ...
    async def notify_run_started(cls, run: Dict[str, Any]):
        ...
        acc_label = (
            ...
            if "t4-x2" in acc
                ...
                "TPU v3-8"
                if "v3-8" in acc
                else ("T4 GPU" if "t4" in acc else "CPU/Default")
            )
        )
