...
class TelegramService:
    ...
    async def send_test_message(cls, test_token: Optional[str] = None, test_chat_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        text = (
            ...
            "Your Telegram notification channel is successfully connected. "
            "You will receive alerts for notebook runs, 12-hour session timeouts, and completions."
        )
        payload = {
            "chat_id": chat_id,
            "text": text,
    ...
    async def notify_run_started(cls, run: Dict[str, Any]):
        ...
        msg = (
            ...
            f"• <b>Notebook:</b> {run.get('title')}\n"
            f"• <b>Account:</b> @{run.get('account_username')}\n"
            f"• <b>Accelerator:</b> <code>{acc_label}</code>\n"
            f"• <b>Max Runtime:</b> 12 Hours\n"
            f"• <b>Kaggle Link:</b> <a href=\"{run.get('kaggle_url')}\">View on Kaggle</a>\n"
        )
    ...
    async def notify_run_completed(cls, run: Dict[str, Any], status: str):
        ...
        msg = (
            f"{icon} <b>Notebook Session {status.upper()}</b>\n\n"
            f"• <b>Notebook:</b> {run.get('title')}\n"
            f"• <b>Account:</b> @{run.get('account_username')}\n"
